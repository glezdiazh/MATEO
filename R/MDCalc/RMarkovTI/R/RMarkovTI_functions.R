...
# ------ FUNCTIONS -------------------------------------------------------------------------------------
RDMarkovMeans <- function(SFile="SMILES.txt",
                          sErrorFile="RDMarkovMeansErrorResults.csv",
                          sResultFile="RDMarkovMeansResults.csv",
                          kPower="3", 
                          atomPropExcluded="Zv,EA", 
                          atomTypesSelected="Het",
                          ruta ="ruta") {
 ...
 #-------------------------------------------------------------------------------
  ...
  dfW=read.table(paste(ruta,"/R/MDCalc/weight/AtomProperties.txt",sep = "", collapse = NULL),header=T)  # read weigths TXT file
  ...
  sError <- data.frame(matrix(ncol = 2, nrow = 0))
  colnames(sError) <- c("Molecule", "SMILE")
  ...
  for(s in 1:nSmiles){          # process for each SMILES
    ...
    {
      sError[nrow(sError) + 1,] = c(molName, smi)
    }
  }
  ...
  write.table(sError, sErrorFile , sep=";", quote = FALSE, row.names=FALSE) 
...
# -----------------------------------------------------------------------------------------------------------------
RDMarkovSingulars <- function(SFile="SMILES.txt",
                              sErrorFile="RDMarkovSingularErrorResults.csv",
                              sResultFile="RDMarkovSingularsResults.csv",
                              kPower=3, atomPropExcluded="Zv,EA", 
                              atomTypesSelected="Het", 
                              fAllKs=0, 
                              ruta = "ruta") {
  ...
  dfW=read.table(paste(ruta,"/R/MDCalc/weight/AtomProperties.txt",sep = "", collapse = NULL),header=T)
